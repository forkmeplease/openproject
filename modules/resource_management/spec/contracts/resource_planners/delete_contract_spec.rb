...
RSpec.describe ResourcePlanners::DeleteContract do
  ...
  let(:resource_planner) { build_stubbed(:resource_planner, project:, principal: owner, public: public_planner) }
  ...
  context "when user is just a viewer" do
    let(:current_user) do
      create(:user, member_with_permissions: { project => %i[view_resource_planners] })
    end
